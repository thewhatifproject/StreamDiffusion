...
class UNetWithControlNet(BaseModel):
    ...
    def get_input_profile(self, batch_size, image_height, image_width, static_batch, static_shape):
        ...
        return {
            ...
            "controlnet_images": [
                (self.num_controlnets, batch_size, 3, min_image_height, min_image_width),
                (self.num_controlnets, batch_size, 3, image_height, image_width),
                (self.num_controlnets, batch_size, 3, max_image_height, max_image_width),
            ],
        }
    ...
    def get_sample_input(self, batch_size, image_height, image_width):
        ...
        # timestep (allowing for 1 to 3 timesteps)
        timestep_input = torch.ones((2 * batch_size,), dtype=torch.float32, device=self.device),
        ...
        controlnets = torch.randn(
                self.num_controlnets, 2 * batch_size, 3, image_height, image_width, dtype=dtype, device=self.device
            )
        
        return sample_input, timestep_input, encoder_hidden_states_input, controlnets

class UNetXLTurboWithControlNet(BaseModel):
    def __init__(
        self,
        fp16=True,
        device="cuda",
        ...
        super(UNetXLTurboWithControlNet, self).__init__(
            fp16=fp16,
            device=device,
            max_batch_size=max_batch_size,
            min_batch_size=min_batch_size,
        ...
        self.name = "UNetXLTurboWithControlNet"
    ...
    def get_input_profile(self, batch_size, image_height, image_width, static_batch, static_shape):
        ...
        return {
            "sample": [
                (2 * batch_size, self.unet_dim, min_latent_height, min_latent_width),
                (2 * batch_size, self.unet_dim, latent_height, latent_width),
                (2 * batch_size, self.unet_dim, max_latent_height, max_latent_width),
            ],
            "timestep": [
                (2 * min_batch,),
                (2 * batch_size,),
            ...
            "encoder_hidden_states": [
                (2 * batch_size, self.text_maxlen, self.encoder_hidden_states_dim),
                (2 * batch_size, self.text_maxlen, self.encoder_hidden_states_dim),
                (2 * batch_size, self.text_maxlen, self.encoder_hidden_states_dim),
            ],
            "text_embeds": [
            ...
            "time_ids": [
                (2 * batch_size, self.time_ids_maxlen),
                (2 * batch_size, self.time_ids_maxlen),
                (2 * batch_size, self.time_ids_maxlen),
            ],
            "controlnet_images": [
                (self.num_controlnets, 2 * batch_size, 3, min_image_height, min_image_width),
                (self.num_controlnets, 2 * batch_size, 3, image_height, image_width),
                (self.num_controlnets, 2 * batch_size, 3, max_image_height, max_image_width),
            ],
        }

    def get_shape_dict(self, batch_size, image_height, image_width):
        # Restituisce le shape finali (batch raddoppiato)
        latent_height, latent_width = self.check_dims(batch_size, image_height, image_width)
        return {
            "sample": (2 * batch_size, self.unet_dim, latent_height, latent_width),
            "timestep": (2, 3),
            "encoder_hidden_states": (2 * batch_size, self.text_maxlen, self.encoder_hidden_states_dim),
            "text_embeds": (2 * batch_size, self.text_embeds_dim),
            "time_ids": (2 * batch_size, self.time_ids_maxlen),
            "controlnet_images": (self.num_controlnets, 2 * batch_size, 3, image_height, image_width),
            "latent": (2 * batch_size, self.unet_dim, latent_height, latent_width),
        }

    def get_sample_input(self, batch_size, image_height, image_width):
        # Genera alcuni tensor random di esempio
        latent_height, latent_width = self.check_dims(batch_size, image_height, image_width)
        dtype = torch.float16 if self.fp16 else torch.float32
        ...
        timestep_input = torch.randint(
            1, 4, (2 * batch_size,), dtype=torch.float32, device=self.device
        )
        encoder_hidden_states_input = torch.randn(
            2 * batch_size, self.text_maxlen, self.encoder_hidden_states_dim, dtype=dtype, device=self.device
        )
        text_embeds_input = torch.randn(
            2 * batch_size, self.text_embeds_dim, dtype=dtype, device=self.device
        )
        time_ids_input = torch.randint(
            0, 1000, (2 * batch_size, self.time_ids_maxlen), dtype=torch.int32, device=self.device
        )
        controlnet_images_input = torch.randn(
            self.num_controlnets, 2 * batch_size, 3, image_height, image_width, dtype=dtype, device=self.device
        )
