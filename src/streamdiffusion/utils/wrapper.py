        ...
        if self.device == "mps":
            ...
            try:
                ...
                if acceleration == "tensorrt":
                    ...
                    from streamdiffusion.acceleration.tensorrt import create_prefix
                    ...
                    unet_engine_path = os.path.join(
                        ...
                        create_prefix(
                            ...
                            vae_batch_size=stream.frame_bff_size #def 1
                        ),
                        "unet.engine",
                    )
                    ...
                    if not os.path.exists(unet_engine_path):
                        raise(f"Acceleration with TensorRT enabled but there is no unet model build at path: {unet_engine_path}")
                    if not os.path.exists(vae_encoder_engine_path):
                        raise(f"Acceleration with TensorRT enabled but there is no vae encoder build at path: {vae_encoder_engine_path}")
                    if not os.path.exists(vae_decoder_engine_path):
                        raise(f"Acceleration with TensorRT enabled but there is no vae decoder model build at path: {vae_decoder_engine_path}")
                    ...
                    if self.is_controlnet_enabled:
                        stream.unet = UNet2DConditionControlNetModelEngine(unet_engine_path, cuda_steram, use_cuda_graph=False)
                    else:
                        stream.unet = UNet2DConditionModelEngine(unet_engine_path, cuda_steram, use_cuda_graph=False)
